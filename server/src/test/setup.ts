...
// ВНИМАНИЕ: Дефолтные значения только для тестового окружения!
// Эти значения НЕ используются в продакшене и безопасны для коммита в репозиторий
process.env.NODE_ENV = 'test';
process.env.API_BASE_URL = process.env.API_BASE_URL || 'http://localhost:4000';
process.env.FRONTEND_URL = process.env.FRONTEND_URL || 'http://localhost:5173';
// Тестовые JWT секреты - используются только в тестах, не в продакшене
...
// Тестовые настройки БД - используются только в тестах
